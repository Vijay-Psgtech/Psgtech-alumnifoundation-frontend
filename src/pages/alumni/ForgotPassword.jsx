    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 px-4 py-16">
      <div className="w-full max-w-md bg-slate-800 border border-yellow-700/10 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-8">
                ...
                return (
                  <React.Fragment key={label}>
                    <div className="flex flex-col items-center gap-2">
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold ${isDone ? "bg-green-100 text-green-700" : isActive ? "bg-amber-400 text-slate-900" : "border border-slate-600 text-slate-400"}`}
                      >
                        {isDone ? "✓" : num}
                      </div>
                      <div
                        className={`text-[10px] tracking-widest uppercase ${isActive ? "text-amber-300" : "text-slate-400"}`}
                      >
                        {label}
                      </div>
                    </div>
                    {i < 2 && (
                      <div
                        className={`flex-1 h-px ${step > num ? "bg-amber-400" : "bg-slate-700"} my-4`}
                        style={{ maxWidth: 44 }}
                      />
                    )}
                  </React.Fragment>
          ...
          {step === STEPS.EMAIL && (
            <>
              <div className="w-14 h-14 rounded-full bg-yellow-50/10 border border-yellow-400/20 flex items-center justify-center mx-auto mb-4 text-2xl">
                🔑
              </div>
              <h3 className="text-xl font-semibold text-slate-100 text-center mb-1">
                Forgot Password?
              </h3>
              ...
              {error && (
                <div className="bg-red-700/10 border border-red-600/20 text-red-200 rounded-md px-3 py-2 mb-3">
                  ⚠ {error}
                </div>
              )}
              <form onSubmit={handleSendOtp}>
                ...
                <button
                  type="submit"
                  className="w-full py-3 rounded-lg bg-gradient-to-r from-amber-400 to-yellow-400 text-slate-900 font-semibold hover:shadow-md disabled:opacity-60"
                  disabled={loading}
                >
              ...
              <Link
                to="/alumni/login"
                className="block text-center text-sm text-slate-400 mt-4 hover:text-slate-200"
              >
                ← Back to Login
              </Link>
            </>
          ...
          {step === STEPS.OTP && (
            <>
              ...
              {info && (
                <div className="bg-green-800/20 border border-green-700/20 text-green-200 rounded-md px-3 py-2 mb-3">
                  ✓ {info}
                </div>
              )}
              ...
              <button
                className="block text-sm text-slate-400 mt-4 mx-auto hover:text-slate-200"
                onClick={() => {
                  setStep(STEPS.EMAIL);
                  setError("");
                }}
          ...
          {step === STEPS.DONE && (
            <div className="text-center py-4">
              <div className="w-16 h-16 rounded-full bg-green-900/20 border border-green-500/30 mx-auto flex items-center justify-center text-3xl mb-4">
                ✓
              </div>
              ...
              <button
                className="w-full py-3 rounded-lg bg-gradient-to-r from-amber-400 to-yellow-400 text-slate-900 font-semibold"
